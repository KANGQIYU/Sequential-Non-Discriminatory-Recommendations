...
def getDataSet(args):
    ...
    fileusers['gender'].replace('F', 0, inplace=True)
    fileusers['gender'].replace('M', 1, inplace=True)
    ...
    onlygender = users.loc[:, ['gender']].copy()
    ...
    users.iloc[:, 0:] = users.iloc[:, 0:].div(users.iloc[:, 0:].sum(axis=1), axis=0)

    pca2 = decomposition.PCA(n_components=2)
    usersdata = np.asarray(users)
    pca2.fit(usersdata)
    newusersdata= pca2.transform(usersdata)

    kmeans = KMeans(n_clusters=args.n_clusters).fit(users)
    labels = kmeans.labels_.astype(int)

    users = pd.DataFrame(newusersdata, index=users.index)
    users['gender'] = onlygender
    # users['cluster_ind'] = labels
    users['cluster_ind'] = 0
    # users.drop("zip_range", axis=1, inplace=True)# use this to drop zip_range
    ...
    # onlygender = fileusers.loc[:, 'gender']
    # fileusers = pd.get_dummies(fileusers)
    ...
    # users = users.loc[:, ['gender_M', 'gender_F', 'cluster_ind']]
