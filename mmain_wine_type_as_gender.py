...
def runone(run_id):
    ...
    num_arms = len(quality)
    # tempid = np.random.choice(num_arms, math.floor(num_arms/2), replace=False)
    # armslastrow = np.zeros([1, num_arms])
    # armslastrow[0, tempid] = 1
    armslastrow = typeindicator.reshape(1, -1)
    ...
    for i in range(args.n_clusters):
        ...
        V_t_unprotect[:, :, i] = X_t_0_unproctect @ X_t_0_unproctect.T + args.lamba_pro * eye(n_feat_dim)
        ...
        V_t_ground[:, :, i] = X_t_0_ground @ X_t_0_ground.T + args.lamba_pro * eye(n_feat_dim)
    ...
    for rtime in range(args.n_trial):
        ...
        if gender == 1:
            r_t_pro = rewards_true[SelectArm_pro, 0] + np.random.choice([0, 1], 1, p=[1-args.pr, args.pr])
        else:
            r_t_pro = rewards_true[SelectArm_pro, 0]
        ...
        if gender == 1:
            r_t_full = rewards_true[SelectArm_full, 0] + np.random.choice([0, 1], 1, p=[1-args.pr, args.pr])
        else:
        ...
        if gender == 1:
            r_t_unprotect = rewards_true[SelectArm_unprotect, 0] + np.random.choice([0, 1], 1, p=[1-args.pr, args.pr])
        else:
...
# Training settings
    ...
    parser.add_argument('--runtimes', type=int, default=1, metavar='N',
    ...
    parser.add_argument('--alpha_pro', type=float, default=0.2, metavar='M',
                        help='set parameter alpha')
    parser.add_argument('--alpha_unprotect', type=float, default=0.2, metavar='M',
                        help='set parameter alpha')
    parser.add_argument('--alpha_full', type=float, default=0.2, metavar='M',
                        help='set parameter alpha')
    parser.add_argument('--alpha_ground', type=float, default=0.2, metavar='M',
                    help='set parameter alpha')
    ...
    n_protect_dim = 1
    n_unproctect_dim = len(list(wine))-1
...
#
    if not args.not_mul:
        ...
        result = np.zeros([args.runtimes, args.n_trial // args.recording_time + 1, 40])
    ...
    regret_true_pro_seq_runs = result[:, :, 4]
    regret_true_full_seq_runs = result[:, :, 5]
    regret_true_unprotect_seq_runs = result[:, :, 6]
    regret_true_ground_seq_runs = result[:, :, 7]
    n_best_true_pro_seq_runs = result[:, :, 8]
    n_best_true_full_seq_runs = result[:, :, 9]
    n_best_true_unprotect_seq_runs =result[:, :, 10]
    n_best_true_ground_seq_runs =result[:, :, 11]
    gender_pro_seq_runs = result[:, :, 12:18]
    gender_full_seq_runs = result[:, :, 18:24]
    gender_unprotect_seq_runs = result[:, :, 24:30]
    gender_ground_seq_runs = result[:, :, 30:36]
    reward_true_pro_seq_runs = result[:, :, 36]
    reward_true_full_seq_runs = result[:, :, 37]
    reward_true_unprotect_seq_runs = result[:, :, 38]
    reward_true_ground_seq_runs = result[:, :, 39]
    ...
    sumr_ground = np.sum(sumr_t_unprotect_seq_runs, axis=0) / n_runtime
