...
def getWine(args):
    ...
    wine = filewhite.iloc[:, 0:-1].copy()
    quality = white_quality

    # x = wine.values  # returns a numpy array
    # min_max_scaler = preprocessing.MinMaxScaler()
    # x_scaled = min_max_scaler.fit_transform(x[:, 0:-1])
    # # wine = pd.DataFrame(x_scaled, index=wine.index, columns=wine.columns)
    # # x_scaled = preprocessing.scale(x[:, 0:-1])
    # wine = pd.DataFrame(x_scaled, index=wine.index)


    # wine = wine.loc[:, ['volatile acidity', 'chlorides']]

    kmeans = KMeans(n_clusters=args.n_clusters).fit(quality)
    ...
    newwinedata = SelectKBest(f_regression, k=11).fit_transform(wine.values, quality.values.ravel())
    wine = pd.DataFrame(newwinedata, index=wine.index)
